...
namespace frp {
    namespace io {
        ...
        int File::GetEncoding(const void* p, int length, int& offset) noexcept {
            offset = 0;
            if (NULL == p || length < 1) {
                return frp::text::Encoding::ASCII;
            }
            ...
            if (s[0] == 0xEF && s[1] == 0xBB && s[2] == 0xBF) {
                offset += 3;
                return frp::text::Encoding::UTF8;
            }
            elif(s[0] == 0xFE && s[1] == 0xFF && s[2] == 0x00) {
                offset += 3;
                return frp::text::Encoding::BigEndianUnicode;
            }
            elif(s[0] == 0xFF && s[1] == 0xFE && s[2] == 0x41) {
                offset += 3;
                return frp::text::Encoding::Unicode;
            }
            return frp::text::Encoding::ASCII;
        }
